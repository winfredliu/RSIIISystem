# serverGOD --> GroundObjectsClassification Server
import time
import logging
...
# 全局变量
gocModel = pdrs.deploy.Predictor('./model/exportedModel/groundObjectsClassification')
progressNum = {}
...
# 模型预热
# gocModel.predict(img_file='./model/warmData/groundObjectsClassification/img.jpg',
#                  warmup_iters=100)
...
@app.route('/')
def index():
    return "Welcome to RSIIISystem's GOD Server!"


# 数据上传
@app.route('/groundObjectsClassification/uploadImg', methods=['POST'])
def gUploadImg():
    ...
    if f:
        ...
        f.save(savePath)
        imgUrl = "http://localhost:5002/groundObjectsClassification/getImage/" + newFilename
...
@app.route('/groundObjectsClassification/uploadZip', methods=['POST'])
def gUploadZip():
    fileDir = os.path.join(basedir, app.config['TMPDIR_FOLDER'])
    if not os.path.exists(fileDir):
        os.makedirs(fileDir)
    ...
    if f:
        ...
        for i in range(length):
            result = model.GOCPredict(imgList[i], gocModel)
            name = getFileName(imgList[i])
            tmpList = model.GOCResultSave(result, name + '.' + extension,
                                          name + '_show.' + extension, dirPath)
            # 将处理进度保存到全局变量中
            progressNum[zipRandomId] = int((i + 1) * 100 / length)
        print(tmpList)
        # 结果压缩
        baseurl = "http://localhost:5002/groundObjectsClassification/download/"
        # 压缩结果数据
...
@app.route('/groundObjectsClassification/getProgressNum', methods=['POST'])
def cGetProgressNum():
...
# 数据处理
@app.route('/groundObjectsClassification/handle', methods=['POST'])
def gHandle():
    ...
    extension = 'png'
    # gocModel = pdrs.deploy.Predictor('./model/exportedModel/groundObjectsClassification')
    result = model.GOCPredict('./tmpDir/' + data['imgName'], gocModel)
    name = create_uuid()
    saveName01 = name + '.' + extension
    saveName02 = name + '_show.' + extension
    resultNameList = model.GOCResultSave(result, saveName01, saveName02)
    print(resultNameList)
    baseurl1 = "http://localhost:5002/groundObjectsClassification/getImage/"
    baseurl2 = "http://localhost:5002/groundObjectsClassification/download/"
...
# 结果下载
@app.route('/groundObjectsClassification/download/<string:filename>', methods=['GET'])
def gDownload(filename):
...
# 展示图片
@app.route('/groundObjectsClassification/getImage/<string:filename>', methods=['GET'])
def gGetImage(filename):
...
if __name__ == "__main__":
    app.run(debug=False, threaded=True, host='localhost', port=5002)

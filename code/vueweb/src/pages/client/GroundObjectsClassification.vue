<template>
  <div id="GroundObjectsClassification">
    <div id="Content">
      <!-- VUE弹窗 -->
      <el-dialog
          id="eDialog"
      ...
      <div id="aLeftSide">
        <el-card
            ...
            style="width: 249px; height: 350px; margin-top: 7px; border-radius:10px;"
        >
          <div slot="header" class="clearfix" style="text-align: center">
            <span class="steps" style="letter-spacing: 7px">操作步骤</span>
          </div>
          <div style="height: 350px;margin-left:40px;" class="stepDiv">
            <el-steps
                direction="vertical"
                :active="stepNum1"
                finish-status="success"
            >
              <el-step style="height: 81px;" title="步骤 1">
                <template slot="description">
                  <!-- 上传文件 -->
                  上传待分类的图片
                  <el-button
              ...
              <el-step style="height: 100px" title="步骤 2">
                <template slot="description">
                  <!-- 发起请求 -->
                  <p>点击分类按钮,</p>
                  <p>发起地物分类请求</p>
                  <el-button
                      ...
                      @click="groundObjectsClassification"
                      class="elBtn"
                  >分类
                  </el-button>
                </template>
              ...
              <!-- 获得图像 -->
              <el-step style="height: 81px" title="步骤 3">
                <template slot="description">
                  <!-- 下载文件 -->
                  下载地物分类结果
                  <el-button
              ...
              <el-step title="获取成功" style="height: 10px">
        ...
        <el-card
            ...
            style="width: 249px; height: 190px; margin-top: 13px; border-radius:10px;"
        >
          <div slot="header" class="clearfix" style="text-align: center">
          ...
          >
            <div class="imagePreview1">
              ...
              <div class="imgInfo" style="border-radius: 0 0 5px 5px">
                <span style="color: white; letter-spacing: 6px">待地物分类图片</span>
              </div>
            </div>
                  ...
                  <div slot="error">
                    <div slot="placeholder" class="error">
                      <el-button
                          ...
                          icon="el-icon-data-analysis"
                          class="elBtn"
                          v-on:click="groundObjectsClassification"
                      >
                        分类
                      </el-button>
                    </div>
...
export default {
  ...
  methods: {
    ...
    updateImg(e) {
      ...
      // 发起上传请求
      axios.post("/groundObjectsClassification/uploadImg", param, config)
          .then((response) => {
    ...
    //地物分类函数
    groundObjectsClassification() {
      if (this.imgUrl == '') {
        this.$message({
          message: '请先上传待进行地物分类的图片',
          type: 'warning'
        });
        return;
      }
      ...
      console.log("开始地物分类");
      // 发起处理请求
      axios.post("/groundObjectsClassification/handle",
          {imgName: getUrlName(this.imgUrl)}, {timeout: 1000 * 60 * 3})
          .then((response) => {
            console.log(response);
            console.log('处理完毕');
    ...
    downloadByName(name) {
      ...
      a.href = '/groundObjectsClassification/download/' + name
      a.download = name
      a.click()
    },
    downloadResultImg() {
    ...
    uploadZip(e) {
      ...
      // 发起上传请求
      axios.post("/groundObjectsClassification/uploadZip", param, config)
          .then((response) => {
            console.log(response);
      ...
      let intervalId = setInterval(() => {
        ...
        axios.post("/groundObjectsClassification/getProgressNum", {zipRandomId: this.zipRandomId})
            .then((response) => {
